# -*- coding: utf-8 -*-
# GPT2_Heuristic_Aligned.py (Manual Loss Eval Fix)
import numpy as np
import random
import os
import time
...
task_max_lengths = {"mrpc": 72, "rte": 150, "mnli": 72, "qqp": 56, "sst2": 32, "qnli": 80} # Keep relatively short lengths

# --- Global Training Configuration (ALIGNED WITH PUDF Script) ---
# Batch sizes from PUDF script. Should be feasible for GPT2 base.
...
def get_example_rarities(list_of_strings):
    ...
    if not all(isinstance(s, str) for s in safe_list):
         raise ValueError("Input must be convertible to a list of strings.")
...
def tokenize_function(examples, task, tokenizer):
    """Tokenizes examples based on task structure and max length for GPT2."""
    max_length = task_max_lengths.get(task, 128)
    # Optionally cap at GPT2's theoretical max (1024), but task-specific limits are likely better
    # max_length = min(max_length, getattr(tokenizer, 'model_max_length', 1024))
    ...
    # Standard Hugging Face pair encoding usually works for GPT2 classification heads
    # The tokenizer automatically adds special tokens if configured (like EOS between pairs)
...
def create_dataset(dataset, task, tokenizer, is_test_set=False):
    """Tokenizes dataset and converts to PyTorch TensorDataset, ensuring difficulty column (GPT2: no TTI)."""
    print(f"Tokenizing dataset (is_test_set={is_test_set})...")
    try:
        # GPT2 doesn't use token_type_ids
        tokenized_cols = ['input_ids', 'attention_mask']
        print("Note: Assuming no token_type_ids generated for GPT2.")
        ...
        # Define final order for GPT2 (no TTI)
...
def evaluate_model(model, dataloader, device, column_order):
    """Evaluates model, calculates loss/accuracy MANUALLY (GPT2: no TTI assumed)."""
    ...
    try: # Determine indices (GPT2: no TTI)
        ...
        print(f"Eval Indices: ids={ids_idx}, mask={mask_idx}, lbl={lbl_idx}") # GPT2: No TTI index
    ...
    for batch in tqdm(dataloader, desc=eval_desc, leave=False):
        ...
        with torch.no_grad(), autocast(device_type='cuda', dtype=amp_eval_dtype, enabled=torch.cuda.is_available()):
            try:
                ...
                outputs = model(input_ids=input_ids, attention_mask=attention_mask) # No labels passed
...
# --- Integrated Curriculum Learning Logic (Corrected for GPT2) ---
def get_epoch_training_data(ts, config, epoch):
    """Selects training data subset based on heuristic scheduler for GPT2 (no TTI)."""
    ...
    # Expected GPT2 structure: ids, mask, labels, difficulty (4 tensors)
    if num_tensors != 4: raise ValueError(f"GPT2 TensorDataset: Expected 4 tensors, got {num_tensors}.")

    input_ids, attention_mask, labels, difficulties = tensors # Direct unpacking
    print(f"[CL Func] Tensor structure: Assumed GPT2 (ids, mask, labels, diff)")
    ...
    # Populate train_sorted (no TTI for GPT2)
...
def train(config, output_dir_base):
    """Main training loop adapted for GPT2 heuristic CL runs with PUDF config."""
    ...
    try: # Model/Tokenizer loading with padding setup for GPT2
        tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir=config.cache_dir, use_fast=True, trust_remote_code=True) # trust_remote_code maybe not needed
        # --- GPT2 Padding Setup ---
        if tokenizer.pad_token is None:
            if tokenizer.eos_token:
                tokenizer.pad_token = tokenizer.eos_token; print(f"Set pad_token to eos_token: {tokenizer.pad_token}")
            else: tokenizer.add_special_tokens({'pad_token': '[PAD]'}); print("Added [PAD] as pad_token.") # Add if no EOS
        # Load model AFTER tokenizer setup
        model = AutoModelForSequenceClassification.from_pretrained(
            model_name,
            num_labels=num_labels,
            cache_dir=config.cache_dir,
            trust_remote_code=True, # Maybe not needed
            pad_token_id=tokenizer.pad_token_id # Explicitly set pad_token_id in config
        )
        # If new token added, resize model embeddings
        if tokenizer.pad_token == '[PAD]': model.resize_token_embeddings(len(tokenizer))
        print(f"Using pad_token_id: {model.config.pad_token_id}")
        # --- End GPT2 Padding ---
    except Exception as e: print(f"FATAL: Loading model/tokenizer failed: {e}"); traceback.print_exc(); return 0.0, 0.0

    model.to(device);
    if config.use_gradient_checkpointing:
        if hasattr(model, "gradient_checkpointing_enable"): model.gradient_checkpointing_enable(); print("Gradient Checkpointing Enabled.")
        else: print("Warning: grad checkpointing enable method not found.")
    if hasattr(model.config, "use_cache") and config.use_gradient_checkpointing: model.config.use_cache = False; print("Disabled use_cache.")
    ...
    for epoch in range(config.num_epochs):
        ...
        if not filtered_data_dict or 'labels' not in filtered_data_dict or len(filtered_data_dict.get('labels', [])) == 0:
            ...
            try: # Create Epoch Loader using expected GPT2 column order
                 # Expected order: ids, mask, labels, difficulty
                 tensors_for_epoch = [filtered_data_dict['input_ids'], filtered_data_dict['attention_mask'], filtered_data_dict['labels'], filtered_data_dict['difficulty']]
                 ...
                 train_dataloader_epoch = DataLoader(train_dataset_epoch, shuffle=True, batch_size=current_epoch_batch_size, **dl_kwargs)
                 print(f"Created epoch dataloader with {len(train_dataloader_epoch)} batches (batch size: {current_epoch_batch_size}).")
        ...
        if train_dataloader_epoch: # Check if dataloader was created
            ...
            for step, batch in enumerate(pbar):
                 ...
                 if len(batch) != 4: print(f"Warning: Unexpected batch length {len(batch)} in train step {step}"); continue
                 try: # Unpack batch
                      input_ids=batch[0].to(device, non_blocking=True); attention_mask=batch[1].to(device, non_blocking=True); labels = batch[2].to(device, non_blocking=True)
                 except Exception as e: print(f"ERROR unpack train batch {step}: {e}"); traceback.print_exc(); continue

                 try: # Training Step
                     with autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
                         # NO token_type_ids for GPT2
                     ...
                     loss_value = loss.item() # Store pre-scaled loss
        ...
        except torch.cuda.OutOfMemoryError: # Catch OOM during validation
    ...
    # Save Training Stats
    model_short = config.model_name.replace('/', '-'); stats_base = f"{model_short}_{config.task}_{config.difficulty_measurer}_{config.training_scheduler}"
    ...
    if test_dataloader is None: print("Test dataloader None. Skip."); test_acc, test_loss = -3.0, -3.0
        ...
        if model_found:
            ...
            try:
                # Load model, trust_remote_code maybe not needed
                model_loaded = AutoModelForSequenceClassification.from_pretrained(best_model_dir).to(device);
    ...
    def convert_np(obj):
        ...
        elif isinstance(obj, types.SimpleNamespace): return vars(obj);
...
# --- run() function with Heuristic Loops & Aligned Config for GPT2 ---
def run():
    # --- Base Configuration (ALIGNED WITH T5 PUDF SCRIPT) ---
    base_config = types.SimpleNamespace()
    base_config.model_name = "gpt2" # *** MODIFIED MODEL NAME ***
    base_config.cache_dir = os.environ.get("TRANSFORMERS_CACHE", "./hf_cache/models")
    base_config.num_epochs = 20 # Matches T5 PUDF
    base_config.learning_rate = 1e-5 # Matches T5 PUDF (Keep for alignment, might need tuning)
    base_config.weight_decay = 0.01 # Matches T5 PUDF (Used in AdamW)
    base_config.warmup_ratio = 0.06 # Matches T5 PUDF
    base_config.max_grad_norm = 1.0 # Matches T5 PUDF
    base_config.gradient_accumulation_steps = gradient_accumulation_steps # Use calculated value (1)
    base_config.gpu = 0 # Matches T5 PUDF
    base_config.num_workers = 4 # Matches T5 PUDF
    base_config.ordering = 'easiest' # Matches T5 PUDF
    base_config.min_train_length = 1000 # Matches T5 PUDF
    base_config.competency = 5 # Keep for linear/root (Not in T5 PUDF)
    base_config.balanced = False # Matches T5 PUDF
    # base_config.use_torch_compile = False # Removed as not used
    base_config.use_gradient_checkpointing = True # Matches T5 PUDF
    base_config.early_stopping_patience = 5 # Matches T5 PUDF
    ...
    model_folder_name = base_config.model_name.replace('/', '-'); # e.g., gpt2
    ...
    for diff_measure in difficulty_measures_to_run:
        for scheduler in schedulers_to_run:
            ...
            # Remove PUDF-specific params if they exist
            if hasattr(config, 'strategy'): delattr(config, 'strategy')
            if hasattr(config, 'diff_dir'): delattr(config, 'diff_dir')
            if hasattr(config, 'num_obs'): delattr(config, 'num_obs')
            if hasattr(config, 'lower_bound'): delattr(config, 'lower_bound')
            if hasattr(config, 'upper_bound'): delattr(config, 'upper_bound')
