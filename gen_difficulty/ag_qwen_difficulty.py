...
models_to_run_from_mapping = [
    "Qwen/Qwen2.5-7B",
    # Add other Qwen models here if needed
]
...
TASK_NAME_FOR_RESULTS_FILE = "ag_news_qwen_20ft_80eval" # Specific for Qwen

MAX_LENGTH = 128
# NUM_LABELS_FROM_DATASET will be determined dynamically

# User-specified batch sizes for Qwen
QWEN_TRAIN_BATCH_SIZE_PER_DEVICE = 32
QWEN_EVAL_BATCH_SIZE_PER_DEVICE = 64

model_train_batch_sizes_per_device = {
    "default": QWEN_TRAIN_BATCH_SIZE_PER_DEVICE, # Default for this script
    "Qwen/Qwen2.5-7B": QWEN_TRAIN_BATCH_SIZE_PER_DEVICE,
}
LLM_TARGET_EFFECTIVE_BATCH_SIZE = 32 # If physical BS is 32, grad accum will be 1

model_eval_batch_sizes_per_device = {
    "default": QWEN_EVAL_BATCH_SIZE_PER_DEVICE, # Default for this script
    "Qwen/Qwen2.5-7B": QWEN_EVAL_BATCH_SIZE_PER_DEVICE,
}

epochs_list = [0, 1, 3, 5, 10]
LEARNING_RATE_ADAFACTOR = 5e-5 # For Adafactor with Qwen
LEARNING_RATE_DEFAULT = 2e-5   # Fallback
...
def baseline_tokenize_for_batch(examples_batch, tokenizer_to_use, max_len_param):
    ...
    if not list_of_texts and "text" not in examples_batch:
        try:
...
for model_checkpoint_id in models_to_run_from_mapping:
    ...
    try:
        ...
        is_qwen_model = "qwen" in model_checkpoint_id.lower() # Should be True here
        tokenizer_instance = AutoTokenizer.from_pretrained(
            model_snapshot_path,
            use_fast=not is_qwen_model, # Qwen usually uses slow tokenizer (use_fast=False)
            trust_remote_code=is_qwen_model # Qwen requires trust_remote_code=True
        )
        print(f"Tokenizer loaded for: {model_checkpoint_id} (Fast: {not is_qwen_model}, TrustRemoteCode: {is_qwen_model})")
        ...
        if tokenizer_instance.pad_token is None:
            if tokenizer_instance.eos_token is not None:
                ...
                # Qwen tokenizers might use <|endoftext|> or similar as EOS and may not have a separate PAD
                # Qwen2 base tokenizer might not have a pad token set by default.
                # Using eos_token as pad_token is a common strategy. If eos_token is also None, then add a new one.
                # For Qwen, often specific tokens like <|extra_0|> can be used if a generic [PAD] conflicts.
                # However, for simplicity, [PAD] or using eos_token if available is standard.
                # Qwen tokenizers might also have unk_token, but eos is preferred for padding if available.
                new_pad_token_str = '<|endoftext|>' # A common EOS/PAD for Qwen like models if eos_token itself is not set
                if tokenizer_instance.eos_token is None and new_pad_token_str in tokenizer_instance.get_vocab():
                     tokenizer_instance.pad_token = new_pad_token_str
                     tokenizer_instance.pad_token_id = tokenizer_instance.convert_tokens_to_ids(new_pad_token_str)
                     print(f"Tokenizer: pad_token set to existing '{new_pad_token_str}'. ID: {tokenizer_instance.pad_token_id}")
                elif tokenizer_instance.eos_token is None : # Add a generic PAD if specific one or EOS not found
        ...
        # QWEN specific: often uses right padding
        # tokenizer_instance.padding_side = "right" # Ensure this is consistent or managed if needed
    ...
    # is_llm_family will be true for Qwen
    ...
    for num_train_epochs_current in epochs_list:
        ...
        try:
            is_qwen_model_check_local = "qwen" in model_checkpoint_id.lower() # Should be True
            config = AutoConfig.from_pretrained(model_snapshot_path, num_labels=NUM_LABELS_FROM_DATASET,
                                                trust_remote_code=is_qwen_model_check_local)
            ...
            elif tokenizer_instance.pad_token_id is None: # This should not happen if tokenizer setup is correct
                # For Qwen, if pad_token_id is still None, it's a critical issue.
                # Qwen models might expect a specific pad_token_id or have it tied to vocab_size.
                # Often, Qwen uses vocab_size as pad_token_id if pad_token is <|endoftext|> or similar and it's the last token.
                # This would require deeper investigation for specific Qwen versions if hit.
                print(f"CRITICAL WARNING: tokenizer.pad_token_id is None for {model_checkpoint_id} before model config sync.")
                if hasattr(config, "vocab_size"): # A potential Qwen-specific fallback, verify if needed
                    # config.pad_token_id = config.vocab_size
                    # print(f"Attempting to set pad_token_id to vocab_size {config.vocab_size} for Qwen as a fallback.")
                    pass


            model_load_kwargs = {"config": config, "trust_remote_code": is_qwen_model_check_local}
            if is_llm_family: # True for Qwen
            ...
            elif model_instance.config.pad_token_id is None and tokenizer_instance.pad_token_id is not None:
                 model_instance.config.pad_token_id = tokenizer_instance.pad_token_id # Set if model's is None
                 print(f"Model config pad_token_id was None, set to tokenizer's: {tokenizer_instance.pad_token_id}")


            if is_llm_family and hasattr(model_instance, 'gradient_checkpointing_enable'): # True for Qwen
            ...
            elif is_llm_family: # True for Qwen
...
print(f"\n===== {TASK_NAME_FOR_RESULTS_FILE.upper()} Experiment (Qwen Models) Completed =====")
