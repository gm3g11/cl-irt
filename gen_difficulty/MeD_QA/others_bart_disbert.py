...
# import shutil # Not actively used in the current minimal cleanup
import traceback
import sys
import time
import inspect  # <<<< ADDED IMPORT FOR ROBUST FORWARD PASS CALLS
...
# Try to import BartForMultipleChoice with robust fallbacks
try:
    from transformers.models.bart.modeling_bart import BartForMultipleChoice

    print("Successfully imported BartForMultipleChoice from transformers.models.bart.modeling_bart")
except ImportError as e_bart_specific:
    print(f"Failed to import BartForMultipleChoice from specific path: {e_bart_specific}")
    try:
        from transformers import BartForMultipleChoice as TopLevelBartForMC

        BartForMultipleChoice = TopLevelBartForMC
        print("Successfully imported BartForMultipleChoice from top-level transformers after all.")
    except ImportError as e_bart_top:
        print(f"Failed to import BartForMultipleChoice from top-level as well: {e_bart_top}")
        print("BartForMultipleChoice class is not available in this Transformers version. BART models will be skipped.")
        BartForMultipleChoice = None  # Sentinel to indicate failure
...
TASK_NAME_FOR_RESULTS_FILE_MC = "MedQA_MC_Encoders_Debug"  # Task name for this script run
...
model_eval_batch_sizes_mc_custom = {"default": 16}
GENERAL_TARGET_EFFECTIVE_BATCH_SIZE_MC = 32
num_epochs_total_mc = 10  # Set to a higher value for a full run, or 1 for quick debug
epochs_to_evaluate_mc = [0, 1, 3, 5, 10]  # Or [0, 1] for quick debug
...
base_output_dir_script = f"./MedQA_MC_Encoders_Debug_run_{current_datetime_str}"
...
def preprocess_encoder_mc_style(examples, tokenizer_instance, max_len):
    ...
    tokenized_output = tokenizer_instance(first_sentences_flat, second_sentences_flat, max_length=max_len,
...
def evaluate_encoder_mc_model(model_to_eval, current_tokenizer, eval_dataset_raw, current_data_collator,
    ...
    tokenized_eval_data = eval_dataset_raw.map(
        ...
        batched=True, num_proc=num_proc_tok_eval, remove_columns=eval_dataset_raw.column_names,
        desc=f"Tokenizing eval data for {model_checkpoint_name_str} epoch {epoch}"
    )
    all_logits_probs, all_responses_correctness, all_true_labels_int = [], [], []
    batch_size_eval = model_eval_batch_sizes_mc_custom.get(model_checkpoint_name_str,
                                                           model_eval_batch_sizes_mc_custom["default"])

    # Get the expected arguments for the model's forward pass
    forward_params = inspect.signature(model_to_eval.forward).parameters
    ...
    for i in tqdm(range(0, len(tokenized_eval_data), batch_size_eval),
        ...
        # Conditionally add token_type_ids if the model accepts it AND it's in the batch
        if "token_type_ids" in forward_params and "token_type_ids" in collated_batch and collated_batch[
            "token_type_ids"] is not None:
            model_inputs["token_type_ids"] = collated_batch["token_type_ids"].to(model_to_eval.device)
        elif "token_type_ids" in collated_batch and collated_batch["token_type_ids"] is not None:
            print(
                f"Info: token_type_ids found in batch but not accepted by {model_to_eval.__class__.__name__}.forward. Not passing.")

        batch_true_labels_int = collated_batch["labels"].numpy()
        all_true_labels_int.extend(batch_true_labels_int)

        with torch.no_grad():
            outputs = model_to_eval(**model_inputs)  # Pass kwargs
        ...
        for pred_idx, true_idx in zip(batch_pred_indices, batch_true_labels_int):
            if true_idx != -100: all_responses_correctness.append(int(pred_idx == true_idx))
    ...
    output_filepath = os.path.join(model_specific_results_dir, output_filename)
...
for model_checkpoint_id in models_to_run:
    ...
    tokenizer_instance, total_training_time_for_run = None, 0.0
    ...
    try:
        ...
        if tokenizer_instance.pad_token is None:
            if tokenizer_instance.eos_token is not None:
                ...
                if new_pad_token_val in tokenizer_instance.vocab and tokenizer_instance.convert_tokens_to_ids(
                        new_pad_token_val) == tokenizer_instance.unk_token_id:
                    new_pad_token_val = "<|pad|>"
                if new_pad_token_val not in tokenizer_instance.get_vocab():
        ...
        elif tokenizer_instance.pad_token_id is not None and tokenizer_instance.pad_token is None:
        ...
        ModelClass = None
        if "distilbert" in model_checkpoint_id.lower():
            ModelClass = DistilBertForMultipleChoice
        elif "bart" in model_checkpoint_id.lower():
            if BartForMultipleChoice is None:
                print(f"FATAL: BartForMultipleChoice could not be imported. Skipping {model_checkpoint_id}.")
                continue
            ModelClass = BartForMultipleChoice
        else:  # Should not be reached if models_to_run is restricted for debug
            raise ValueError(
                f"Model type {model_checkpoint_id} not explicitly handled for ForMultipleChoice class in this debug run.")
        ...
        config_epoch0 = AutoConfig.from_pretrained(model_checkpoint_id, num_labels=NUM_CHOICES_MC,
                                                   cache_dir=os.environ["TRANSFORMERS_CACHE"])
        if tokenizer_instance.pad_token_id is not None and hasattr(config_epoch0,
                                                                   'pad_token_id') and config_epoch0.pad_token_id != tokenizer_instance.pad_token_id:
            config_epoch0.pad_token_id = tokenizer_instance.pad_token_id
        model_epoch0 = ModelClass.from_pretrained(model_checkpoint_id, config=config_epoch0,
                                                  cache_dir=os.environ["TRANSFORMERS_CACHE"])
        if added_new_pad_token_flag and hasattr(model_epoch0, 'resize_token_embeddings') and len(
                tokenizer_instance) > model_epoch0.config.vocab_size:
            model_epoch0.resize_token_embeddings(len(tokenizer_instance))
        if hasattr(model_epoch0.config,
                   'pad_token_id') and model_epoch0.config.pad_token_id != tokenizer_instance.pad_token_id and tokenizer_instance.pad_token_id is not None:
        ...
        evaluate_encoder_mc_model(model_epoch0, tokenizer_instance, medqa_dataset_for_evaluation_raw, data_collator_mc,
                                  0, 0.0, model_checkpoint_id, TASK_NAME_FOR_RESULTS_FILE_MC, model_results_dir)
        del model_epoch0
        ...
        config_train = AutoConfig.from_pretrained(model_checkpoint_id, num_labels=NUM_CHOICES_MC,
                                                  cache_dir=os.environ["TRANSFORMERS_CACHE"])
        if tokenizer_instance.pad_token_id is not None and hasattr(config_train,
                                                                   'pad_token_id') and config_train.pad_token_id != tokenizer_instance.pad_token_id:
            config_train.pad_token_id = tokenizer_instance.pad_token_id
        model_for_training = ModelClass.from_pretrained(model_checkpoint_id, config=config_train,
                                                        cache_dir=os.environ["TRANSFORMERS_CACHE"])
        if added_new_pad_token_flag and hasattr(model_for_training, 'resize_token_embeddings') and len(
                tokenizer_instance) > model_for_training.config.vocab_size:
            model_for_training.resize_token_embeddings(len(tokenizer_instance))
        if hasattr(model_for_training.config,
                   'pad_token_id') and model_for_training.config.pad_token_id != tokenizer_instance.pad_token_id and tokenizer_instance.pad_token_id is not None:
        ...
        training_args = TrainingArguments(
            output_dir=model_checkpoints_dir, overwrite_output_dir=True, num_train_epochs=num_epochs_total_mc,
            per_device_train_batch_size=current_train_bs, gradient_accumulation_steps=grad_accum,
            learning_rate=LEARNING_RATE_MC, weight_decay=0.01, save_strategy="epoch", eval_strategy="no",
        ...
        trainer = Trainer(model=model_for_training, args=training_args, train_dataset=tokenized_medqa_for_sft,
                          tokenizer=tokenizer_instance, data_collator=data_collator_mc)
        ...
        for desired_epoch in epochs_to_evaluate_mc:
            ...
            if os.path.exists(model_checkpoints_dir):
                ckpt_folders = sorted(
                    [os.path.join(model_checkpoints_dir, d) for d in os.listdir(model_checkpoints_dir) if
                     d.startswith("checkpoint-") and os.path.isdir(os.path.join(model_checkpoints_dir, d))])
            ...
            if found_ckpt and ckpt_path_to_load:
                ...
                if tokenizer_instance.pad_token_id is not None and hasattr(config_ckpt,
                                                                           "pad_token_id") and config_ckpt.pad_token_id != tokenizer_instance.pad_token_id:
                    config_ckpt.pad_token_id = tokenizer_instance.pad_token_id
                model_checkpoint = ModelClass.from_pretrained(ckpt_path_to_load, config=config_ckpt)
                if hasattr(model_checkpoint.config,
                           'pad_token_id') and model_checkpoint.config.pad_token_id != tokenizer_instance.pad_token_id and tokenizer_instance.pad_token_id is not None:
                ...
                evaluate_encoder_mc_model(model_checkpoint, tokenizer_instance, medqa_dataset_for_evaluation_raw,
                                          data_collator_mc, desired_epoch, total_training_time_for_run,
