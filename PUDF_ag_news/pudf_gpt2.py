...
PER_DEVICE_TRAIN_BATCH_SIZE_CONFIG = 256
GRADIENT_ACCUMULATION_STEPS_CONFIG = 1
...
# --- Helper: Create GPT-2 Tensor Dataset ---
def create_gpt2_tensor_dataset(hf_tokenized_dataset_split,
                               partition_name="unknown",
                               include_difficulty=True):
    print(f"Creating TensorDataset for GPT-2 {partition_name} (include_difficulty={include_difficulty})...")
    ...
    if 'token_type_ids' in actual_columns and 'token_type_ids' in hf_tokenized_dataset_split.features:
        ...
        print(f"  INFO ({partition_name}): 'token_type_ids' found and will be included.")
    else:
        print(f"  INFO ({partition_name}): 'token_type_ids' will NOT be included (expected for GPT-2 AGNews).")
...
# --- Helper: Evaluate GPT-2 Theta ---
def evaluate_and_estimate_gpt2_theta(model, dataloader, device, column_order, num_labels,
                                     num_obs_theta=-1, desc_prefix="GPT2ThetaEst"):
    print(f"Estimating GPT-2 Theta ({desc_prefix})...")
    model.eval()
    ...
    try:
        ...
        print(f"FATAL ERROR ({desc_prefix}): Column missing for theta. Order: {column_order}. Error: {e}")
        traceback.print_exc();
        return 0.0, 0.0
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=desc_prefix, leave=False):
            try:
                ...
                token_type_ids_batch = batch[token_type_ids_idx].to(device,
                                                                    non_blocking=True) if token_type_ids_idx != -1 else None
            except Exception as e:
...
# --- Helper: Evaluate GPT-2 on Main Validation (Acc, Loss) ---
def evaluate_gpt2_main_val(model, dataloader, device, column_order, num_labels, desc_prefix="GPT2MainVal"):
    print(f"Evaluating GPT-2 on Main Validation ({desc_prefix})...")
    model.eval()
    ...
    try:
        ...
        print(f"FATAL ({desc_prefix}): Column missing. Error: {e}");
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=desc_prefix, leave=False):
            ...
            try:
                ...
                token_type_ids_batch = batch[token_type_ids_idx].to(device) if token_type_ids_idx != -1 else None
    ...
    if dataset_has_difficulty and all_difficulties_main_val and all_logits_for_theta and len(all_logits_for_theta) > 0:
        ...
        try:
            logits_c, labels_c, diffs_c = map(np.concatenate,
                                              [all_logits_for_theta, all_labels, all_difficulties_main_val])
            if len(logits_c) == len(labels_c) == len(diffs_c) and len(labels_c) > 0:
                ...
                theta_main = \
                calculate_theta(diffs_c, rps, PUDF_CONFIG.num_obs_theta if PUDF_CONFIG.num_obs_theta > 0 else -1)[
                    0]  # Ensure num_obs logic
...
def train_gpt2_pudf():
    ...
    try:
        ...
        if len(diff_scores) != len(train_full_hf): raise ValueError("Difficulty scores mismatch.")
    ...
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, cache_dir=os.environ["TRANSFORMERS_CACHE"], use_fast=True)
    ...
    if num_cpus is not None:
        num_proc_map = min(num_cpus, 32)  # Use up to 32 cores if available
    else:
        num_proc_map = 4  # Fallback
    num_proc_map = max(1, num_proc_map)  # Ensure at least 1
    ...
    cols_to_remove_base = [col for col in complete_ds.column_names if col not in ['text', 'label', 'difficulty']]

    for name, ds_split in [("actual_train", actual_train_hf), ("theta_estimation", theta_estimation_hf),
                           ("validation_main", validation_main_hf), ("test", test_hf)]:
        current_cols_to_remove = ["text"] + [col for col in cols_to_remove_base if col in ds_split.column_names]
        tokenized_datasets_dict[name] = ds_split.map(tokenize_fn, batched=True, num_proc=num_proc_map,
                                                     remove_columns=current_cols_to_remove)
        if 'label' in tokenized_datasets_dict[name].column_names:  # Rename to 'labels'
    ...
    actual_train_tds, actual_train_col_order = create_gpt2_tensor_dataset(tokenized_datasets_dict["actual_train"],
                                                                          "actual_train_pool", True)
    theta_est_tds, theta_est_cols = create_gpt2_tensor_dataset(tokenized_datasets_dict["theta_estimation"],
                                                               "theta_est_set", True)
    theta_est_dl = DataLoader(theta_est_tds, PER_DEVICE_EVAL_BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True)

    main_val_has_diff = 'difficulty' in tokenized_datasets_dict["validation_main"].column_names
    val_main_tds, val_main_cols = create_gpt2_tensor_dataset(tokenized_datasets_dict["validation_main"],
                                                             "val_main_eval", main_val_has_diff)
    main_val_dl = DataLoader(val_main_tds, PER_DEVICE_EVAL_BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True)
    ...
    else:  # This should have been handled by tokenizer setup
        raise ValueError("Tokenizer pad_token_id is None, cannot configure model.")

    model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, config=model_config,
                                                               torch_dtype=model_load_dtype)

    if hasattr(model, 'resize_token_embeddings') and len(tokenizer) > model.config.vocab_size:  # If '[PAD]' was added
        print(f"Resizing model token embeddings to: {len(tokenizer)}")
        model.resize_token_embeddings(len(tokenizer))
        # Ensure pad_token_id in config is still correct after resize
    ...
    num_approx_update_steps_epoch = ceil(
        len(actual_train_tds) / (PER_DEVICE_TRAIN_BATCH_SIZE_CONFIG * GRADIENT_ACCUMULATION_STEPS_CONFIG))
    ...
    print(f"GradScaler enabled: {scaler.is_enabled()} (use_bf16: {use_bf16})")

    print(f"\nStarting PUDF GPT-2 Training Loop ({PUDF_CONFIG.strategy} strategy)...")
    best_val_acc = 0.0;
    early_stop_count = 0;
    train_stats = []
    pudf_overhead = 0.0;
    ...
    for epoch in range(NUM_TRAIN_EPOCHS):
        ...
        if PUDF_CONFIG.strategy == 'theta':
            ...
            est_theta_guidance, theta_est_t = evaluate_and_estimate_gpt2_theta(model, theta_est_dl, device,
                                                                               theta_est_cols, num_labels,
                                                                               PUDF_CONFIG.num_obs_theta,
                                                                               f"Ep{epochs_completed} ThetaEst")
            model.train();  # Ensure model is back in train mode after eval in theta estimation
        ...
        pudf_args.epoch = epoch  # get_epoch_training_data often expects 0-indexed epoch

        # CORRECTED: Call to get_epoch_training_data with all required positional arguments
        theta_hat_for_filtering = cur_cap if PUDF_CONFIG.strategy in ['theta', 'theta-hard'] and isinstance(cur_cap,
                                                                                                            float) else None
        filtered_data = get_epoch_training_data(
            actual_train_tds,  # 1. TensorDataset
            actual_train_col_order,  # 2. Column order list
            pudf_args,  # 3. Config object
            epoch,  # 4. Epoch int
            PUDF_CONFIG.task_name_for_pudf,  # 5. Task string
            theta_hat_for_filtering,  # 6. Capacity float (theta_hat for the function)
            lower_offset=PUDF_CONFIG.lower_bound,
        ...
        if n_epoch_samples == 0:
            ...
            # For GPT-2 AGNews, no token_type_ids are expected in filtered_data if not in actual_train_tds
            # TensorDataset order: input_ids, attention_mask, labels, difficulty
            epoch_tensors = [filtered_data['input_ids'], filtered_data['attention_mask'],
                             filtered_data['labels'], filtered_data['difficulty']]

            epoch_ds_filtered = TensorDataset(*epoch_tensors)
            epoch_dl = DataLoader(epoch_ds_filtered, PER_DEVICE_TRAIN_BATCH_SIZE_CONFIG, shuffle=True, num_workers=2,
                                  pin_memory=True)
            ...
            for step, batch_data in enumerate(prog_bar):
                # Batch order based on epoch_tensors: ids, mask, labels, difficulty
                ...
                with autocast(device.type, dtype=amp_dtype,
                    ...
                    # GPT2ForSequenceClassification does not use token_type_ids for single sentence input
                ...
                loss_val_acc = loss_val / GRADIENT_ACCUMULATION_STEPS_CONFIG
                ...
                if (step + 1) % GRADIENT_ACCUMULATION_STEPS_CONFIG == 0 or (step + 1) == len(epoch_dl):
        ...
        val_acc, val_loss_val, theta_main, theta_main_t = evaluate_gpt2_main_val(model, main_val_dl, device,
        ...
        if val_acc > best_val_acc:
            print(f"Val acc improved ({best_val_acc:.4f} -> {val_acc:.4f}). Saving best model...");
    ...
    print(f"Total training loop duration: {total_loop_t:.2f}s ({total_loop_t / 3600:.2f} hours)")  # ADDED PRINT

    stats_p = os.path.join(OUTPUT_DIR, "training_statistics.json");
    ...
    test_accuracy, test_loss = -10.0, -10.0  # Default if test eval doesn't run
    ...
    if os.path.isdir(best_model_p):
        if (os.path.exists(os.path.join(best_model_p, "pytorch_model.bin")) or
            ...
            if any(fname.startswith("pytorch_model-") or fname.startswith("model-") for fname in files_in_dir) and \
                ...
                print(f"Found sharded model files in {best_model_p}, attempting to load.")
                can_load_model = True
            elif files_in_dir and any(f.endswith((".json", ".bin", ".safetensors")) for f in files_in_dir):
                print(
                    f"Warning: {best_model_p} exists but no standard weight/index files matched strongly. Attempting load from dir.")
                can_load_model = True

    if can_load_model:
        print("\nLoading best model for test eval...")
        try:
            model_config_best = AutoConfig.from_pretrained(best_model_p)
            model = AutoModelForSequenceClassification.from_pretrained(best_model_p, config=model_config_best,
                                                                       torch_dtype=model_load_dtype).to(device)

            test_tds, test_cols = create_gpt2_tensor_dataset(tokenized_datasets_dict["test"], "test_final", False)
            test_dl = DataLoader(test_tds, PER_DEVICE_EVAL_BATCH_SIZE, num_workers=2, pin_memory=True)
            test_accuracy, test_loss, _, _ = evaluate_gpt2_main_val(model, test_dl, device, test_cols, num_labels,
                                                                    "Final Test Eval")
            print(f"Final Test Results: Acc={test_accuracy:.4f}, Loss={test_loss:.4f}")
        except Exception as e_load_test:
            print(f"Error during best model test evaluation: {e_load_test}")
            traceback.print_exc()
            test_accuracy, test_loss = -1.0, -1.0  # Error during test eval

    else:
        print("No valid best model found for test set evaluation.")
        test_accuracy, test_loss = -2.0, -2.0  # Model not found/saved
    ...
    cfg_log.update({'LEARNING_RATE': LEARNING_RATE, 'TRAIN_BATCH_SIZE_PHYSICAL': PER_DEVICE_TRAIN_BATCH_SIZE_CONFIG,
                    'GRAD_ACCUM_STEPS': GRADIENT_ACCUMULATION_STEPS_CONFIG,
                    'EVAL_BATCH_SIZE': PER_DEVICE_EVAL_BATCH_SIZE})
    summary = {"best_validation_accuracy": best_val_acc, "final_test_accuracy": test_accuracy,
               "final_test_loss": test_loss, "epochs_run": epochs_completed,
               "total_training_loop_time_seconds": round(total_loop_t, 2),
               "total_pudf_overhead_time_seconds": round(pudf_overhead, 2), "config": cfg_log}
    ...
    print("===== PUDF GPT-2 AGNews Task Finished =====")
