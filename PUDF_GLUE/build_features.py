...
def get_epoch_training_data(ts, args, epoch, task, theta_hat=None, diffs_sorted_idx=None, lower_offset=-np.inf, upper_offset=0):
    ...
    if args.use_word_rarity:
        assert ts.tensors[4] is not None  # Ensure 'example_rarity' is included
    ...
    # Extract tensors
    input_ids, attention_mask, token_type_ids, labels, difficulties = training_set.tensors

    # determine difficulties if needed
    if args.use_length:
        difficulties = torch.tensor([len(p) for p in input_ids])  # Adjust if necessary

    if args.use_word_rarity:
        difficulties = training_set.tensors[4]  # Assuming 'example_rarity' is the 5th tensor

    if diffs_sorted_idx is None:
        diffs_sorted_idx = np.argsort(difficulties.cpu().numpy())

    if args.ordering == 'easiest':
        diffs_sorted_idx = np.argsort(difficulties.cpu().numpy())
    elif args.ordering == 'hardest':
        diffs_sorted_idx = np.argsort(difficulties.cpu().numpy())[::-1]
    elif args.ordering == 'middleout':  # middle out
        diffs_sorted_idx = np.argsort(np.abs(difficulties.cpu().numpy()))
    else:  # random baseline
        raise NotImplementedError

    # determine if we want balanced per-label or not
    if not args.balanced:
        train_2['input_ids'] = input_ids[diffs_sorted_idx]
        train_2['attention_mask'] = attention_mask[diffs_sorted_idx]
        train_2['token_type_ids'] = token_type_ids[diffs_sorted_idx]
        train_2['labels'] = labels[diffs_sorted_idx]
        train_2['difficulty'] = difficulties[diffs_sorted_idx]
    else:
        per_label_lists = {}
        ...
        train_2['input_ids'] = input_ids[train_2_idx]
        train_2['attention_mask'] = attention_mask[train_2_idx]
        train_2['token_type_ids'] = token_type_ids[train_2_idx]
        train_2['labels'] = labels[train_2_idx]
        train_2['difficulty'] = difficulties[train_2_idx]

    # based on strategy, select our training set for this epoch
    if args.strategy == 'ordered':
        return train_2
    elif args.strategy == 'simple':
        data_per_epoch = len(input_ids) / (args.num_epochs / 2.)
        if epoch % 2 == 0:
            num_train = min(int(data_per_epoch * (epoch + 1)), len(input_ids))
        else:
            num_train = min(int(data_per_epoch * (epoch)), len(input_ids))
        train['input_ids'] = train_2['input_ids'][:num_train]
        train['attention_mask'] = train_2['attention_mask'][:num_train]
        train['token_type_ids'] = train_2['token_type_ids'][:num_train]
        train['labels'] = train_2['labels'][:num_train]
        train['difficulty'] = train_2['difficulty'][:num_train]
        return train
    elif args.strategy == 'theta':
        train_idx = [i for i in range(len(input_ids)) if (theta_hat + lower_offset) <= train_2['difficulty'][i] <= (theta_hat + upper_offset)]
        if len(train_idx) < args.min_train_length:
            train_idx = [i for i in range(args.min_train_length)]
        train['input_ids'] = train_2['input_ids'][train_idx]
        train['attention_mask'] = train_2['attention_mask'][train_idx]
        train['token_type_ids'] = train_2['token_type_ids'][train_idx]
        train['labels'] = train_2['labels'][train_idx]
        train['difficulty'] = train_2['difficulty'][train_idx]
        return train
    elif args.strategy == 'theta-hard':
        train_idx = [i for i in range(len(input_ids)) if train_2['difficulty'][i] >= theta_hat]
        if len(train_idx) < args.min_train_length:
            train_idx = [i for i in range(args.min_train_length)]
        train['input_ids'] = train_2['input_ids'][train_idx]
        train['attention_mask'] = train_2['attention_mask'][train_idx]
        ...
        num_train = int(epoch_competency * len(input_ids))
        train['input_ids'] = train_2['input_ids'][:num_train]
        train['attention_mask'] = train_2['attention_mask'][:num_train]
        train['token_type_ids'] = train_2['token_type_ids'][:num_train]
        train['labels'] = train_2['labels'][:num_train]
...
def load_glue_task(datadir, diffdir, taskname):
    ...
    # load difficulties
    train_diff_file = '{}/{}1pl/best_parameters.json'.format(diffdir, taskname.lower())
    # Read the JSON file
    with open(train_diff_file, 'r') as file:
        data = json.load(file)

    train['difficulty'] = data['diff']
    ...
    sss = StratifiedShuffleSplit(1, test_size=0.1, random_state=21)
...
def get_example_rarities(examples):
    ...
    if single_sentence:
        tokenized = [tokenize(e[0].strip()).split(' ') for e in examples]
    else:
        tokenized = [tokenize((e[0] + e[1]).strip()).split(' ') for e in examples]
