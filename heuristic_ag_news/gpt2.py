# -*- coding: utf-8 -*-
# GPT-2 Heuristic Script with Custom Sampler Fix
...
MAX_LEN = 128
random_seed = 63
# Training Params specific to GPT-2 AG News script
LEARNING_RATE = 2e-5
...
# Feature Flags
USE_BF16 = True # Enable if supported

# --- Root Output Directory ---
BASE_OUTPUT_DIR_ROOT = "./gpt2_agnews_heuristic_runs_customsampler" # Updated dir name
...
class HeuristicSampler(Sampler[int]):
    # ... (implementation unchanged from T5/Llama/DeBERTa fix) ...
    def __init__(self, num_samples_total: int, batch_size: int, sorted_indices: list[int], heuristic_config: dict, num_replicas: int = 1, rank: int = 0, seed: int = 42):
        ...
        self.abs_min_train_length = self.heuristic_config.get('abs_min_train_length', 0) # Allow absolute min if needed
...
# --- Custom Trainer Definition (No Optimizers Needed for GPT-2 Default) ---
class CustomHeuristicTrainer(Trainer):
    def __init__(self, *args, sorted_indices=None, heuristic_config=None, num_samples_total=None, **kwargs):
        # GPT-2 script uses default optimizer, so call parent WITHOUT optimizers
...
# --- Main Training Function for GPT-2 (Using Custom Sampler) ---
def run_training_heuristic_gpt2(difficulty_measurer, training_scheduler, output_dir_run):
    """ Runs heuristic loop for GPT-2 with Custom Sampler. """
    global tokenizer
    print("\n" + "="*80); print(f"Starting GPT-2 Run: Diff='{difficulty_measurer}', Sched='{training_scheduler}'"); print(f"Output Dir: {output_dir_run}"); print("="*80 + "\n")
    ...
    try:
        ...
        if len(train_labels_unique) > num_labels_expected:
            ...
            num_labels = num_labels_expected # Use the expected number after filtering
        else:
            print("Dataset seems to have expected number of labels. Skipping filtering.")
            dataset_filtered = dataset_raw
            num_labels = len(train_labels_unique) # Use actual number found
        ...
        # 4. Load GPT-2 Tokenizer & Set Padding
        print(f"Loading tokenizer: {MODEL_NAME}")
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, use_fast=False)
        if tokenizer.pad_token is None:
            print("Setting GPT-2 padding token to EOS token.")
        ...
        columns_for_model = ['input_ids', 'attention_mask', 'labels'] # GPT-2 doesn't use token_type_ids
        ...
        # Resize embeddings if pad token was added (might be needed if EOS wasn't already pad)
        # model.resize_token_embeddings(len(tokenizer)) # Uncomment if needed
        ...
        training_args = TrainingArguments(
            ...
            # gradient_checkpointing=False, # Usually not needed for GPT-2 base
        ...
        print(f"\n!!! ERROR during GPT-2 run: Diff='{difficulty_measurer}', Sched='{training_scheduler}' !!!"); print(f"Output Dir: {output_dir_run}"); traceback.print_exc()
        return {"error": str(e)}
    finally:
        # 15. Cleanup
...
# --- Experiment Loop ---
overall_results = {}
print("\n===== Starting GPT-2 (Custom Sampler) Experiment Loops =====") # Updated print
for diff_measure in difficulty_measures_to_run:
    for scheduler in schedulers_to_run:
        ...
        run_results = run_training_heuristic_gpt2(diff_measure, scheduler, current_output_dir) # Call correct func
        overall_results[run_id] = run_results
        time.sleep(2)

# --- Overall Summary ---
print("\n\n===== Overall GPT-2 (Custom Sampler) Experiment Summary =====") # Updated print
...
print("\nAll GPT-2 (Custom Sampler) experiment runs completed.") # Updated print