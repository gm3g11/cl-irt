...
import evaluate
# No BeautifulSoup needed for ag_news
import warnings
...
from huggingface_hub import whoami # Optional login check
...
MAX_LEN = 128
random_seed = 63
# Training Params specific to DeBERTa AG News script
LEARNING_RATE = 1e-5
TRAIN_BATCH_SIZE_PER_DEVICE = 256
EVAL_BATCH_SIZE_PER_DEVICE = 256
NUM_TRAIN_EPOCHS = 20 # Used for max_steps calculation
WEIGHT_DECAY = 0.01
LOGGING_STEPS = 10
...
# Feature Flags
USE_BF16 = True # DeBERTa v3 often benefits from bf16

# --- Root Output Directory ---
BASE_OUTPUT_DIR_ROOT = "./deberta_agnews_heuristic_runs_customsampler" # Updated dir name
...
class HeuristicSampler(Sampler[int]):
    ...
    def __init__(self, num_samples_total: int, batch_size: int, sorted_indices: list[int], heuristic_config: dict, num_replicas: int = 1, rank: int = 0, seed: int = 42):
        ...
        # Include absolute min length if provided in config (though not currently set for DeBERTa)
    ...
    def set_epoch(self, epoch: int) -> None:
        ...
        if self.num_replicas > 1:
            num_samples_per_replica = len(self.indices_for_epoch) // self.num_replicas
            self.num_samples_epoch_replica = num_samples_per_replica
            total_size = self.num_samples_epoch_replica * self.num_replicas
            self.indices_for_epoch = self.indices_for_epoch[:total_size]
...
# --- Custom Trainer Definition (No Optimizers Needed for DeBERTa Default) ---
class CustomHeuristicTrainer(Trainer):
    def __init__(self, *args, sorted_indices=None, heuristic_config=None, num_samples_total=None, **kwargs):
        # DeBERTa script doesn't use custom optimizer, so call parent WITHOUT optimizers
...
def compute_metrics(p):
    ...
    valid_idx = labels != -100 # Standard ignore index
    if not np.any(valid_idx): return {"accuracy": 0.0}
    preds = preds[valid_idx]; labels = labels[valid_idx]
    return accuracy_metric.compute(predictions=preds, references=labels)

# --- Main Training Function for DeBERTa (Using Custom Sampler) ---
def run_training_heuristic_deberta(difficulty_measurer, training_scheduler, output_dir_run):
    """ Runs heuristic loop for DeBERTa with Custom Sampler. """
    global tokenizer
    print("\n" + "="*80); print(f"Starting DeBERTa Run: Diff='{difficulty_measurer}', Sched='{training_scheduler}'"); print(f"Output Dir: {output_dir_run}"); print("="*80 + "\n")
    ...
    try:
        ...
        # Assuming 4 classes are desired (0, 1, 2, 3) based on T5 setup. Apply filter.
        num_labels = 4 # Explicitly set desired number of labels
        print(f"Filtering dataset splits to keep labels 0-{num_labels-1}...")
        dataset_filtered = dataset_raw.filter(lambda x: 'label' in x and 0 <= x['label'] < num_labels, num_proc=NUM_PREPROCESSING_PROCS)
        ...
        # Verify num_labels matches filtered data if needed
        actual_labels = train_dataset_filtered.unique('label')
        if len(actual_labels) > num_labels: print(f"Warning: Filtered data still has {len(actual_labels)} unique labels. Check filtering.")
        num_labels = len(actual_labels) # Use actual number of labels post-filtering
        ...
        # 4. Load DeBERTa Tokenizer
        print(f"Loading tokenizer: {MODEL_NAME}")
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, use_fast=False) # As per original script

        # 5. Tokenize Datasets
        print(f"Tokenizing datasets...")
        # Keep 'label', remove 'difficulty' and 'text' after tokenization
        ...
        # DeBERTa v3 *might* use token_type_ids
        columns_for_model = ['input_ids', 'attention_mask', 'labels']
        if 'token_type_ids' in tokenized_train_full.column_names:
            columns_for_model.append('token_type_ids')
        ...
        # 7. Load DeBERTa Model
        print(f"Loading model: {MODEL_NAME} (Num Labels: {num_labels})")
        model = AutoModelForSequenceClassification.from_pretrained(
            MODEL_NAME, num_labels=num_labels, torch_dtype=amp_dtype, # Load with target dtype
            cache_dir=os.environ["TRANSFORMERS_CACHE"]
        )
        # No pad token config usually needed for DeBERTa
        ...
        total_train_batch_size = TRAIN_BATCH_SIZE_PER_DEVICE * world_size * GRADIENT_ACCUMULATION_STEPS
        ...
        training_args = TrainingArguments(
            ...
            eval_strategy="epoch", save_strategy="epoch", learning_rate=LEARNING_RATE,
            ...
            # gradient_checkpointing=False, # DeBERTa might not need/support it easily
            ...
            # remove_unused_columns=True # Set True, shouldn't matter with manual cleaning
        )
        ...
        # 10. Initialize CustomHeuristicTrainer
        ...
        trainer = CustomHeuristicTrainer(
            ...
            tokenizer=tokenizer,
            # No optimizers tuple needed for default AdamW
            # Pass custom args
        ...
        print(f"\n!!! ERROR during DeBERTa run: Diff='{difficulty_measurer}', Sched='{training_scheduler}' !!!"); print(f"Output Dir: {output_dir_run}"); traceback.print_exc()
        return {"error": str(e)}
    finally:
        # 15. Cleanup
        print(f"Cleaning up resources for run: {output_dir_run}")
        del model, trainer, dataset_filtered
...
for diff_measure in difficulty_measures_to_run:
    for scheduler in schedulers_to_run:
        ...
        run_results = run_training_heuristic_deberta(diff_measure, scheduler, current_output_dir)
