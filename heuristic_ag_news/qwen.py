# -*- coding: utf-8 -*-
# Qwen Heuristic Script with Custom Sampler Fix
...
import warnings
import packaging # Use packaging for version check
import gc
import re
...
from huggingface_hub import login, whoami # For HF login check

# --- Static Configuration (Doesn't change across runs) ---
MODEL_NAME = "Qwen/Qwen2.5-7B"
DATASET_NAME = "contemmcm/ag_news"
MAX_LEN = 128
random_seed = 63
# Training Params specific to Qwen AG News script
LEARNING_RATE = 1e-5 # Base LR for TrainingArguments
ADAFACOR_LR = 2e-5   # Specific LR for Adafactor optimizer
PER_DEVICE_TRAIN_BATCH_SIZE = 128 # From Qwen script
PER_DEVICE_EVAL_BATCH_SIZE = 128 # From Qwen script
GRADIENT_ACCUMULATION_STEPS = 16 # From Qwen script (Effective BS = 128 * 16 = 2048)
NUM_TRAIN_EPOCHS = 15 # From Qwen script (Used for max_steps calc)
WEIGHT_DECAY = 0.01
LOGGING_STEPS = 100
GRADIENT_CHECKPOINTING = True # From Qwen script
USE_BF16 = True # From Qwen script (assumes compatible hardware)
METRIC_FOR_BEST = "eval_accuracy"
EARLY_STOPPING_PATIENCE = 3
# Acceleration Settings
NUM_PREPROCESSING_PROCS = max(1, os.cpu_count() // 2)
DATALOADER_WORKERS = 4
# Fixed Heuristic Params
ORDERING = 'easiest'
COMPETENCY_PARAM = 5
...
BASE_OUTPUT_DIR_ROOT = "./qwen_7b_agnews_heuristic_runs_customsampler" # Updated dir name
...
except Exception as e: print(f"HF Login Check Failed: {e}."); hf_token_available = False
else: hf_token_available = True
...
# --- Custom Trainer Definition (Accepts Optimizers for Qwen/Adafactor) ---
class CustomHeuristicTrainer(Trainer):
    def __init__(self, *args, sorted_indices=None, heuristic_config=None, num_samples_total=None, optimizers=None, **kwargs): # Accept optimizers
    ...
    def get_train_dataloader(self) -> DataLoader:
        # ... (implementation unchanged from T5/Llama fix) ...
...
# --- Main Training Function for Qwen (Using Custom Sampler) ---
def run_training_heuristic_qwen(difficulty_measurer, training_scheduler, output_dir_run):
    """ Runs heuristic loop for Qwen with Custom Sampler. """
    global tokenizer
    print("\n" + "="*80); print(f"Starting Qwen Run: Diff='{difficulty_measurer}', Sched='{training_scheduler}'"); print(f"Output Dir: {output_dir_run}"); print("="*80 + "\n")
    os.makedirs(output_dir_run, exist_ok=True)
    model, trainer = None, None
    adafactor_optimizer, optimizers = None, None # For Qwen's optimizer
    ...
    try:
        ...
        # 2. *** ADD Label Filtering (Check if needed for contemmcm/ag_news) ***
        num_labels_expected = 4 # Standard AG News
        train_labels_unique = dataset_raw['train'].unique('label')
        print(f"Unique labels found in raw train data: {train_labels_unique}")
        if len(train_labels_unique) > num_labels_expected:
            print(f"Filtering dataset splits to keep labels 0-{num_labels_expected-1}...")
            dataset_filtered = dataset_raw.filter(lambda x: 'label' in x and 0 <= x['label'] < num_labels_expected, num_proc=NUM_PREPROCESSING_PROCS)
            num_labels = num_labels_expected
        else:
            print("Dataset seems to have expected number of labels. Skipping filtering.")
            dataset_filtered = dataset_raw
            num_labels = len(train_labels_unique)
        train_dataset_filtered = dataset_filtered['train']; validation_dataset_filtered = dataset_filtered['validation']; test_dataset_filtered = dataset_filtered['test']
        ...
        if len(difficulty_scores) != len(train_dataset_final_raw): raise RuntimeError("Mismatch scores and final data size.")
        ...
        del train_dataset_filtered; gc.collect()

        # 4. Load Qwen Tokenizer & Set Padding
        print(f"Loading tokenizer: {MODEL_NAME}")
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
        if tokenizer.pad_token is None:
            print("Setting Qwen padding token to EOS token.")
        ...
        columns_for_model = ['input_ids', 'attention_mask', 'labels'] # Qwen likely doesn't use token_type_ids
        ...
        # 7. Load Qwen Model & Set Config
        print(f"Loading model: {MODEL_NAME} (Num Labels: {num_labels})")
        print(f"Loading model with dtype: {amp_dtype}")
        model = AutoModelForSequenceClassification.from_pretrained(
            MODEL_NAME, num_labels=num_labels, torch_dtype=amp_dtype,
            cache_dir=os.environ["TRANSFORMERS_CACHE"],
            # attn_implementation="flash_attention_2" # Optional: if installed and SDPA warning persists
        )
        ...
        training_args = TrainingArguments(
            ...
            eval_strategy="epoch", save_strategy="epoch", learning_rate=LEARNING_RATE, # Base LR
            per_device_train_batch_size=PER_DEVICE_TRAIN_BATCH_SIZE,
            per_device_eval_batch_size=PER_DEVICE_EVAL_BATCH_SIZE,
            gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS,
            # num_train_epochs=NUM_TRAIN_EPOCHS, # Use max_steps
            max_steps=calculated_max_steps,      # *** SET MAX STEPS ***
            weight_decay=WEIGHT_DECAY,
            logging_dir=os.path.join(output_dir_run, "logs"), logging_steps=LOGGING_STEPS,
        ...
        # 10. Define Adafactor Optimizer (as used in original Qwen script)
        print("Using Adafactor optimizer.")
        adafactor_optimizer = Adafactor(
            model.parameters(), scale_parameter=False, relative_step=False,
            lr=ADAFACOR_LR, # Use the specific Adafactor LR from config
            warmup_init=False, weight_decay=WEIGHT_DECAY
        )
        optimizers = (adafactor_optimizer, None) # Trainer handles scheduler based on args
        ...
        heuristic_config_dict = {
            ...
            "c_init": C_INIT, # Add abs_min_train_length if used by sampler
        }
        ...
        trainer = CustomHeuristicTrainer(
            ...
            tokenizer=tokenizer,
            optimizers=optimizers, # *** Pass Adafactor optimizer tuple ***
            # Pass custom args
        ...
        if test_dataset_for_eval is not None and len(test_dataset_for_eval) > 0:
            ...
            print("Test results:", test_results)
             ...
             print("Test set is empty after filtering."); test_results = {"eval_accuracy": 0.0, "status": "empty"}
        ...
        print(f"\n!!! ERROR during Qwen run: Diff='{difficulty_measurer}', Sched='{training_scheduler}' !!!"); print(f"Output Dir: {output_dir_run}"); traceback.print_exc()
        return {"error": str(e)}
    finally:
        # 17. Cleanup
...
# --- Experiment Loop ---
overall_results = {}
print("\n===== Starting Qwen (Custom Sampler) Experiment Loops =====") # Updated print
for diff_measure in difficulty_measures_to_run:
    for scheduler in schedulers_to_run:
        ...
        run_results = run_training_heuristic_qwen(diff_measure, scheduler, current_output_dir) # Call correct func
        overall_results[run_id] = run_results
        time.sleep(2)

# --- Overall Summary ---
print("\n\n===== Overall Qwen (Custom Sampler) Experiment Summary =====") # Updated print
for run_id, results in overall_results.items():
     print(f"\n--- Results for Run: {run_id} ---")
     if "error" in results: print(f"  ERROR: {results['error']}")
     elif "eval_accuracy" in results: # Check metric
         ...
         if results.get("status") == "empty": print("  (Test set was empty after filtering)")
     else: print(f"  Test results format unknown: {results}")
print("==========================================")
overall_summary_file = os.path.join(BASE_OUTPUT_DIR_ROOT, "overall_summary_customsampler.json")
try:
...
print("\nAll Qwen (Custom Sampler) experiment runs completed.") # Updated print